...
namespace Server.Gumps
{
	public class DeclareCityAllegiancesGump : Gump
	{
		...
		public DeclareCityAllegiancesGump( CityManagementStone stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new DeclareCityAllegiancesGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}
				...
				else
				{
					if ( !m_Stone.AllegiancesDeclared.Contains( stone ) )
					{
						...
						{
							m.SendMessage( "CITY MESSAGE: We have declared allegiance with {0}.", stone.CityName );
						}
					}
					...
					if ( !stone.AllegiancesInvited.Contains( m_Stone ) )
					{
						...
						{
							m.SendMessage( "CITY MESSAGE: {0} has declared allegiance with us.", m_Stone.CityName );
						}
					}
				}	
			}
