...
{
	public class SleeperSmallSouth: BaseAddon, IChopable
	{
      ...
      public override BaseAddonDeed Deed
        {
            get
            {
                return new SleeperSmallSouthDeed ();
            }
        }

		public SleeperSmallSouth( Serial serial ) : base( serial )
		{
		}

		[Constructable]
		public SleeperSmallSouth( ) 
		{
			Visible = true;
			Name = "Sleeper";
		...
		private SleeperSmallSouth m_Sleeper;
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public SleeperSmallSouth Sleeper
		{
			...
			else
			{
				...
				{	
					...
					if ( m_house!= null && (m_house.IsOwner(from) || m_house.IsCoOwner(from) || m_house.IsFriend( from )))
					{
						...
						Point3D m_Location = new Point3D(this.Location.X, this.Location.Y+2, this.Location.Z+8);
	...
	public class SleeperSmallSouthPiece : AddonComponent
	{

		private SleeperSmallSouth m_Sleeper;

		public SleeperSmallSouthPiece( SleeperSmallSouth sleeper, int itemid ) : base( itemid )
		{
			m_Sleeper = sleeper;
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			base.Deserialize( reader );
			int version = reader.ReadInt();
			m_Sleeper = (SleeperSmallSouth)reader.ReadItem();

		}

	}
   public class SleeperSmallSouthDeed : BaseAddonDeed
    {
        public override BaseAddon Addon
        {
            get
            {
                return new SleeperSmallSouth ();
            }
        }

        [Constructable]
        public SleeperSmallSouthDeed ()
        {
            Name = "a small sleepable bed facing south deed.";
        }

        public SleeperSmallSouthDeed ( Serial serial )
            : base ( serial )
        {
        }
