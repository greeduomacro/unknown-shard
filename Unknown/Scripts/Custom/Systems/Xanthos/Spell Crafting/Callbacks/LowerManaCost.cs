		...
		public static void Callback( Mobile from, object target, object craftState )
		{
			...
			try
			{
				if ( target is BaseArmor && !( target is BaseShield ) )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseArmor)target, AosAttribute.LowerManaCost, m_Minimum, m_Maximum );
					...
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseJewel)target, AosAttribute.LowerManaCost, m_Minimum, m_Maximum );
