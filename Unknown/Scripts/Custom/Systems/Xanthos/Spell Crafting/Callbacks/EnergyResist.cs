		...
		public static void Callback( Mobile from, object target, object craftState )
		{
			...
			try
			{
				if ( target is BaseArmor )
					SpellCraft.ApplyResistance( from, cs.Book, cs.Id, (BaseArmor)target, ResistanceType.Energy, m_Minimum, m_Maximum );
				
				else if ( target is BaseWeapon )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseWeapon)target, AosWeaponAttribute.ResistEnergyBonus, m_Minimum, m_Maximum );
				
				else if ( target is BaseHat )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseHat)target, AosElementAttribute.Energy, m_Minimum, m_Maximum );

				else if ( target is BaseJewel ) 
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseJewel)target, AosElementAttribute.Energy, m_Minimum, m_Maximum );
