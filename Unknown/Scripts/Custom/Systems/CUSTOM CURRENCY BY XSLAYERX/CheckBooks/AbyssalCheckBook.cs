...
{	
	public class AbyssalCoinCheckBook : Item 
	{
		private int m_Token;
		...
		[Constructable]
		public AbyssalCoinCheckBook() : base( 0xff0 )
		{
			Movable = true;
			Weight = 0;
			Name = "Abyssal Coin Check book";
			LootType = LootType.Blessed;
			Hue = 37;			
		}
		
		[Constructable]
		public AbyssalCoinCheckBook( Mobile m ) : base( 0xff0 )
		{
			Movable = true;
			Weight = 0;
			Name = "Abyssal Coin Check book";
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendGump( new AbyssalCoinCheckBookGump( (PlayerMobile)from, this ) );
			}
		}

		public void BeginCombine( Mobile from )
		{
			from.Target = new AbyssalCoinCheckBookTarget( this );
		}
		...
		public void EndCombine( Mobile from, object o )
		{
			...
			{
				if (!( o is Abyssalcoin ) && !( o is AbyssalCoinBankCheck ) )
				{
				...
				if ( o is AbyssalCoinBankCheck )
				{
					...
					{
						AbyssalCoinBankCheck AbyssalCoinBankCheck = o as AbyssalCoinBankCheck;
						Token += AbyssalCoinBankCheck.Worth;
						AbyssalCoinBankCheck.Delete();
						from.SendGump( new AbyssalCoinCheckBookGump( (PlayerMobile)from, this ) );
						BeginCombine( from );
					}
				}
		...
		public AbyssalCoinCheckBook( Serial serial ) : base( serial )
		{
		}
...
//		}
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060658, "Abyssal Coin Check book balance\t{0}", m_Token );
			
			if( m_Owner != null )
			list.Add( 1060659, "Abyssal Coin Check book owner\t{0}", m_Owner.Name );
		}
...
namespace Server.Gumps
{
	...
	public class AbyssalCoinCheckBookGump : Gump
	{
	
		private PlayerMobile m_From;
		private AbyssalCoinCheckBook m_Book;	
      	   
		public AbyssalCoinCheckBookGump( PlayerMobile from, Item item ): base( 0, 0 )
		{
		
			m_From = from;			
			if (!(item is AbyssalCoinCheckBook))
				return;
			AbyssalCoinCheckBook book = item as AbyssalCoinCheckBook;
			m_Book = book;

			m_From.CloseGump( typeof( AbyssalCoinCheckBookGump ) );
...
//			AddTextEntry(68, 216, 242, 20, 0, 1, "0");
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1)
			{
				m_From.SendGump( new AbyssalCoinCheckBookGump( m_From, m_Book ) );
			...
			if ( info.ButtonID == 2 )
			{
				...
				if(tr_TokenAmount != null)
				{
					...
					if(i_MaxAmount > 0) 
					{
						if (i_MaxAmount <= ((AbyssalCoinCheckBook)m_Book ).Token)
						{				
							if (i_MaxAmount <= 60000 )
							{

								m_From.AddToBackpack(new Abyssalcoin(i_MaxAmount));
								m_From.SendMessage(1161, "You extracted {0} Abyssal Coins from your check book.", i_MaxAmount);
								((AbyssalCoinCheckBook)m_Book ).Token = (((AbyssalCoinCheckBook)m_Book ).Token - i_MaxAmount);

							}
							else
								m_From.SendMessage(1161, "You can't extract more then 60,000 Abyssal Coins at one time.");
						}
						else
							m_From.SendMessage(1173, "You don't have that many Abyssal Coins in your book.");
					}
					m_From.SendGump( new AbyssalCoinCheckBookGump( m_From, m_Book ) );
				}
			}
				...
				if(tr_TokenAmount != null)
				{
					...
					if(i_MaxAmount > 0) 
					{
						...
						{				
							...
							{
								m_From.AddToBackpack(new AbyssalCoinBankCheck(i_MaxAmount));
								m_From.SendMessage(1161, "You extracted {0} Abyssal Coins from your check book.", i_MaxAmount);
								((AbyssalCoinCheckBook)m_Book ).Token = (((AbyssalCoinCheckBook)m_Book ).Token - i_MaxAmount);
							}
							else
								m_From.SendMessage(1161, "You cannnot write checks for more then 1,000,000 Abyssal Coins at one time.");
						}
						else
							m_From.SendMessage(1173, "You don't have that much Abyssal Coins in your check book.");
					}
					m_From.SendGump( new AbyssalCoinCheckBookGump( m_From, m_Book ) );
				}
			}
...
namespace Server.Items
{
	public class AbyssalCoinCheckBookTarget : Target
	{
		private AbyssalCoinCheckBook m_Book;

		public AbyssalCoinCheckBookTarget( AbyssalCoinCheckBook book ) : base( 18, false, TargetFlags.None )
		{
			m_Book = book;
		}
