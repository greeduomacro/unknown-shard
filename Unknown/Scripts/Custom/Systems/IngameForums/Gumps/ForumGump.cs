...
namespace Server.Forums
{
	public class ForumGump : Gump
	{
		...
		public ForumGump( Mobile pm, int page ) : base( 0, 0 )
		{
            m_Player = pm;
            m_Page = page;
            ...
            AddLabel( 435, 507, 0, "Close" );
            AddLabel( 131, 507, 0, "New Post" );
            AddLabel( 613, 161, 499, "Credits" );
            AddLabel( 614, 187, 499, "Help" );
            AddLabel( 87, 68, 499, "Sorious' In-Game Forums - Total Threads: " + ForumCore.Threads.Count.ToString() + " Total Moderators: " + ForumCore.Moderators.Count.ToString() );
            AddLabel( 87, 93, 499, "Users Online: " + NetState.Instances.Count );
            AddLabel( 535, 101, 499, "Version " + ForumCore.Version );
            AddLabel( 80, 124, 499, "   Posts       Subject                           Views/Replys" );
            ...
            for( int i = index; i < ForumCore.Threads.Count; i++ )
            {
                ...
                if( !te.Deleted && !( te.StaffMessage && m_Player.AccessLevel < AccessLevel.Counselor ) )
                {
                    ...
                    AddButtonLabeled( 100, ( ( ( 35 * ( i - index ) ) + 155 ) - offset ), ( i - ( maxcount - ( ( ( m_Page + 1 ) * 10 ) ) ) - index ), 
                                      buttonID, te.Subject, te.NumberOfViews.ToString(), te.NumberOfReplys.ToString(), te.StaffMessage, te.EntryType );
                }
                else
                {
                    maxcount++;
        ...
        public void AddButtonLabeled( int x, int y, int buttonID, int buttonValue, string text, string views, string replys, bool staff, ThreadType type )
        {
            ...
            if( type == ThreadType.Sticky )
                AddHtml( x + 40, y, 300, 20, Color( "Sticky: " + text, LabelColor - 450 ), false, false );
