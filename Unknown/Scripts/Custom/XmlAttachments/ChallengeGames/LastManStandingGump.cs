...
namespace Server.Gumps
{
	public class LastManStandingGump : Gump
	{
		...
		public LastManStandingGump( LastManStandingGauntlet gauntlet, Mobile from, int page ) : base( 20, 30 )
		{
			...
			if(gauntlet.Participants != null)
			{
                ...
                for(int i = 0;i<m_WorkingList.Count;i++)
                {
                    ...
                    LastManStandingGauntlet.ChallengeEntry entry = (LastManStandingGauntlet.ChallengeEntry)m_WorkingList[i];
            ...
            if(gauntlet.Winner != null)
            {
                AddLabel( 130, height - 35, 68, String.Format(XmlPoints.GetText(from, 200534), gauntlet.Winner.Name) ); // "{0} is the winner!"
            }
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
                case 1:
                    ...
                    state.Mobile.SendGump( new LastManStandingGump( m_gauntlet, state.Mobile, viewpage));
                    break;
				...
				default:
				{
				    ...
				    if(info.ButtonID >= 4000)
                    {
                        ...
                        {
                            LastManStandingGauntlet.ChallengeEntry entry = (LastManStandingGauntlet.ChallengeEntry)m_WorkingList[selection];
                            ...
                            if(m_gauntlet.Participants != null)
                            {
                                ...
                                foreach(LastManStandingGauntlet.ChallengeEntry masterentry in m_gauntlet.Participants)
                                {
                            ...
                            if(m_gauntlet.Participants != null)
                            {
                                ...
                                if(kickentry != null)
                                {
                                    ...
                                    if(kickentry.Participant != null)
                                    {
                                        ...
                                        kickentry.Participant.SendGump( new LastManStandingGump( m_gauntlet, kickentry.Participant, viewpage));
                                    }
                                }
                            }
		...
		private class MemberTarget : Target
        {
            ...
            public MemberTarget(LastManStandingGauntlet gauntlet, ArrayList list) :  base ( 30, false, TargetFlags.None )
            {
                m_list = list;
                m_gauntlet = gauntlet;
            }
