	...
	public class BandageContext
	{
		...
		public void EndHeal()
		{
			...
			else
			{
				...
				if ( chance > Utility.RandomDouble() )
				{
					...
					if ( Core.AOS )
					{
						min = (anatomy / 8.0) + (healing / 4.0) + 8.0;
						max = (anatomy / 5.0) + (healing / 1.5) + 8.0;
					}
					else
					{
						min = (anatomy / 5.0) + (healing / 4.0) + 6.0;
						max = (anatomy / 3.0) + (healing / 2.0) + 20.0;
					}
		...
		public static BandageContext BeginHeal( Mobile healer, Mobile patient )
		{
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				{
					if ( Core.AOS )
						seconds = 3.0 + (0.5 * ((double)(120 - dex) / 10)); // TODO: Verify algorithm
					else
						seconds = 5.4 + (0.6 * ((double)(120 - dex) / 10));
				}
				else
				{
