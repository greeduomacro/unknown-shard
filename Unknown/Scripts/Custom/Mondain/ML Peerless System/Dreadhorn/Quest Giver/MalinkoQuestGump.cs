...
{
	public class MalinkoQuestGump : Gump
	{
		public MalinkoQuestGump( Mobile m): base( 0, 0 )
		{
			this.Closable=false;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch( info.ButtonID )
			{
				case 0:
				{
					...
					from.CloseGump( typeof( MalinkoQuestGump ) );
					from.AddToBackpack(new MalinkoMarker());
					from.AddToBackpack(new MalinkoQuestBook());

					break;
				}
				case 1:
				{
					from.SendMessage( "Oh well, fuck off then wimp!" );
					from.CloseGump( typeof( MalinkoQuestGump ) );
					break;
				}
			}
