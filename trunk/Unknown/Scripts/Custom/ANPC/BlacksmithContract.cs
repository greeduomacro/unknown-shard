...
{
	public class ABlacksmithContract : Item
	{

		[Constructable]
		public ABlacksmithContract() : base( 0x2831 )
		{
		    Hue = 1368;
		    Name = "Blacksmith Contract";
			Weight = 0.1;
			LootType = LootType.Blessed;
		}

		public ABlacksmithContract( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( from.AccessLevel >= AccessLevel.GameMaster )
			{
				...
				Mobile v = new ABlacksmith();//( from, BaseHouse.FindHouseAt( from ) );
			...
			else
			{
				...
				else
				{
					...
					{
						Mobile v = new ABlacksmith();//( from, house );
