...
{
	public class FreezeDipTub : BaseTub
	{
		...
		[Constructable]
		public FreezeDipTub() : this ( 50, 0 )
		{
		}
		
		[Constructable]
		public FreezeDipTub( int charges, int owner ) : base ( 0xFAB )
		{
			Name = "Freeze Dip Tub";
			Charges = charges;
			Owner = owner;
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsChildOf( from.Backpack ) )
			{
				if (i_Charges >= 1 )
					...
					from.SendMessage( "You don't have enough charges to freeze anymore." );
			}
		}
		
		public FreezeDipTub( Serial serial ) : base( serial )
		{
		}
		...
		{
			private FreezeDipTub it_Tub;
			
			public InternalTarget( FreezeDipTub tub ) : base( 1, false, TargetFlags.None )
			{
				it_Tub = tub;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted != null && targeted is Arrow )
				{
					if ( it_Tub != null )
					{
						...
						{
							from.SendMessage( "You carefully apply the cold material to your bundle of arrows." );
							
							it_Tub.Charges--;
							arrow.Consume( 20 );
							//Note: pack.ConsumeTotal( typeof(Arrow), 20 )
							from.AddToBackpack( new FreezeArrow( 20 ) );
						}
				...
				else if ( targeted != null && targeted is Bolt )
				{
					if ( it_Tub != null )
					{
						...
						{
							from.SendMessage( "You carefully apply the cold material to your bundle of bolts." );
							
							it_Tub.Charges--;
							bolt.Consume( 20 );
							//Note: pack.ConsumeTotal( typeof(Bolt), 20 )
							from.AddToBackpack( new FreezeBolt( 20 ) );
						}
