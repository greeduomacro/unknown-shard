...

{
	public class CheeseForm : Item
	{
		...
		private int FromAfaire; // 1-sheep 2-goat 3-cow
		private int FromageQual; //10:nothing 11-89:normal 90-99:magic
		private int StadeFermentation; // display 0% to 100% fermentation for impatient ones
		private bool MoulePlein; // Cheeseform is filled. Dclick again for sarting the fermentation process
		private bool Fermentation; // Cheeseform in in fermentation (true) or not (false)
		private bool ContientUnFromton;// Fermented cheeseform (true) or empty (false)
		public int m_FromBonusSkill; // bonus for skill cooking .
		...
		public override void OnDoubleClick (Mobile from )
		{
			...
			else
			{
				...
				else if ((MoulePlein == true) && (Fermentation == false) && (ContientUnFromton == false))
				{
					...
					Fermentation = true;
                    from.SendMessage(0x84C, "You start the fermentation process.");
				...
				{
                    from.SendMessage(0x84C, "Fermentation process: " + StadeFermentation.ToString() + " %");
				}
				else if ((Fermentation == false) && (ContientUnFromton == true) && (MoulePlein == true))
				{
					if (m_FromBonusSkill < 10)
					{
                        from.SendMessage(0x84C, "Fermentation failed, the milk is lost.");
