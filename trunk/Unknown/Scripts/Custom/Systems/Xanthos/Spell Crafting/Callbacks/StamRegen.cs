...
namespace Server.SpellCrafting.Crafts
{
	public class RegenStam
	{
		...
		private static readonly int m_Maximum = 3;
		...
		public static void Callback( Mobile from, object target, object craftState )
		{
			...
			try
			{
				if ( target is BaseArmor && !( target is BaseShield ) )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseArmor)target, AosAttribute.RegenStam, m_Minimum, m_Maximum );
				
				else if ( target is BaseHat )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseHat)target, AosAttribute.RegenStam, m_Minimum, m_Maximum );
