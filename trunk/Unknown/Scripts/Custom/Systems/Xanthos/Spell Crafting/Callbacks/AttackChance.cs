		...
		public static void Callback( Mobile from, object target, object craftState )
		{
			...
			try
			{
				if ( target is BaseShield )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseShield)target, AosAttribute.AttackChance, m_Minimum, m_Maximum );
				
				else if ( target is BaseWeapon )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseWeapon)target, AosAttribute.AttackChance, m_Minimum, m_Maximum );
				
				else if ( target is BaseJewel )
					SpellCraft.ApplyAttribute( from, cs.Book, cs.Id, (BaseJewel)target, AosAttribute.AttackChance, m_Minimum, m_Maximum );
