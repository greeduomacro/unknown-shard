...
{	
	public class MooniteCheckBook : Item 
	{
		private int m_Token;
		...
		[Constructable]
		public MooniteCheckBook() : base( 0xff0 )
		{
			Movable = true;
			Weight = 0;
			Name = "Moonite Coin Check book";
			LootType = LootType.Blessed;
			Hue = 3;			
		}
		
		[Constructable]
		public MooniteCheckBook( Mobile m ) : base( 0xff0 )
		{
			Movable = true;
			Weight = 0;
			Name = "Moonite Coin Check book";
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendGump( new MooniteCheckBookGump( (PlayerMobile)from, this ) );
			}
		}

		public void BeginCombine( Mobile from )
		{
			from.Target = new MooniteCheckBookTarget( this );
		}
		...
		public void EndCombine( Mobile from, object o )
		{
			...
			{
				if (!( o is Moonitecoin ) && !( o is MooniteBankCheck ) )
				{
				...
				if ( o is MooniteBankCheck )
				{
					...
					{
						MooniteBankCheck MooniteBankCheck = o as MooniteBankCheck;
						Token += MooniteBankCheck.Worth;
						MooniteBankCheck.Delete();
						from.SendGump( new MooniteCheckBookGump( (PlayerMobile)from, this ) );
						BeginCombine( from );
					}
				}
		...
		public MooniteCheckBook( Serial serial ) : base( serial )
		{
		}
...
//		}
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1060658, "Moonite Coin Check book balance\t{0}", m_Token );
			
			if( m_Owner != null )
			list.Add( 1060659, "Moonite Coin Check book owner\t{0}", m_Owner.Name );
		}
...
namespace Server.Gumps
{
	...
	public class MooniteCheckBookGump : Gump
	{
	
		private PlayerMobile m_From;
		private MooniteCheckBook m_Book;	
      	   
		public MooniteCheckBookGump( PlayerMobile from, Item item ): base( 0, 0 )
		{
		
			m_From = from;			
			if (!(item is MooniteCheckBook))
				return;
			MooniteCheckBook book = item as MooniteCheckBook;
			m_Book = book;

			m_From.CloseGump( typeof( MooniteCheckBookGump ) );
...
//			AddTextEntry(68, 216, 242, 20, 0, 1, "0");
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1)
			{
				m_From.SendGump( new MooniteCheckBookGump( m_From, m_Book ) );
			...
			if ( info.ButtonID == 2 )
			{
				...
				if(tr_TokenAmount != null)
				{
					...
					if(i_MaxAmount > 0) 
					{
						if (i_MaxAmount <= ((MooniteCheckBook)m_Book ).Token)
						{				
							if (i_MaxAmount <= 60000 )
							{

								m_From.AddToBackpack(new Moonitecoin(i_MaxAmount));
								m_From.SendMessage(1161, "You extracted {0} Moonite Coins from your check book.", i_MaxAmount);
								((MooniteCheckBook)m_Book ).Token = (((MooniteCheckBook)m_Book ).Token - i_MaxAmount);

							}
							else
								m_From.SendMessage(1161, "You can't extract more then 60,000 Moonite Coins at one time.");
						}
						else
							m_From.SendMessage(1173, "You don't have that many Moonite Coins in your book.");
					}
					m_From.SendGump( new MooniteCheckBookGump( m_From, m_Book ) );
				}
			}
				...
				if(tr_TokenAmount != null)
				{
					...
					if(i_MaxAmount > 0) 
					{
						...
						{				
							...
							{
								m_From.AddToBackpack(new MooniteBankCheck(i_MaxAmount));
								m_From.SendMessage(1161, "You extracted {0} Moonite Coins from your check book.", i_MaxAmount);
								((MooniteCheckBook)m_Book ).Token = (((MooniteCheckBook)m_Book ).Token - i_MaxAmount);
							}
							else
								m_From.SendMessage(1161, "You cannnot write checks for more then 1,000,000 Moonite Coins at one time.");
						}
						else
							m_From.SendMessage(1173, "You don't have that much Moonite Coins in your check book.");
					}
					m_From.SendGump( new MooniteCheckBookGump( m_From, m_Book ) );
				}
			}
...
namespace Server.Items
{
	public class MooniteCheckBookTarget : Target
	{
		private MooniteCheckBook m_Book;

		public MooniteCheckBookTarget( MooniteCheckBook book ) : base( 18, false, TargetFlags.None )
		{
			m_Book = book;
		}
