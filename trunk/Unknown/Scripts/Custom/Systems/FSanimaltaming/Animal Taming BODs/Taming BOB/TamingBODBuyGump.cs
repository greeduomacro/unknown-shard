...
{
	public class TamingBODBuyGump : Gump
	{
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			if ( info.ButtonID == 2 )
			{
				...
				if ( m_Book.Entries.Contains( m_Object ) && pv != null )
				{
					...
					{
						if ( m_Object is TamingBOBLargeEntry )
							price = ((TamingBOBLargeEntry)m_Object).Price;
						else if ( m_Object is TamingBOBSmallEntry )
							price = ((TamingBOBSmallEntry)m_Object).Price;
					}
					...
					else
					{
						...
						if ( m_Object is TamingBOBLargeEntry )
							item = ((TamingBOBLargeEntry)m_Object).Reconstruct();
						else if ( m_Object is TamingBOBSmallEntry )
							item = ((TamingBOBSmallEntry)m_Object).Reconstruct();
