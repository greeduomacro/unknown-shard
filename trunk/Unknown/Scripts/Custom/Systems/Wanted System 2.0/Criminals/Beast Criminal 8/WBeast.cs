...
{ 
	[CorpseName( "a Beasts corpse" )] 
	public class WBeast : BaseCreature 
	{
    ...
    private static bool m_Talked; // flag to prevent spam 
      ...
      { 
         "Im going to incert my harddrive in your face and download it into your ass", 
         "Damn Bitch you need to call 1=800-Jenny",   
         "Im going to fuck you up so bad Dr. Phil cant help you",
};
		[Constructable] 
		public WBeast() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 ) 
		{ 
			
			Name = "Beast the Magician";
			Body = 400;
			Hue = 0;  
			...
			Beastshirt Chest = new Beastshirt();
			Chest.Movable = false;
			AddItem(Chest); 
			
			BeastCloak BaseCloak = new BeastCloak();
			BaseCloak.Movable = false;
			AddItem(BaseCloak);
			
			BeastPike BaseSpear = new BeastPike();
			BaseSpear.Movable = false;
			AddItem(BaseSpear);
			
			BeastBoots BaseShoes = new BeastBoots();
			BaseShoes.Movable = false;
			AddItem(BaseShoes);

			BeastPants Legs = new BeastPants();
			Legs.Movable = false;
			AddItem(Legs);				
      ...
      { 
		...
		public WBeast( Serial serial ) : base( serial )
		{
		}
