...
namespace Server.Gumps
{
	public class CancelAllegiancesGump : Gump
	{
		...
		public CancelAllegiancesGump( CityManagementStone stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new CancelAllegiancesGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}
				...
				if ( m_Stone.Allegiances.Contains( stone ) )
				{
					...
					{
						m.SendMessage( "CITY MESSAGE: We are no longer allied with {0}.", stone.CityName );
					}
				}
				...
				if ( stone.Allegiances.Contains( m_Stone ) )
				{
					...
					{
						m.SendMessage( "CITY MESSAGE: We are no longer allied with {0}.", m_Stone.CityName );
					}
				}	
			}
		}
