...
namespace Server.Gumps
{
	public class ViewCityBansGump : Gump
	{
		...
		public ViewCityBansGump( CityManagementStone stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new ViewCityBansGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}
