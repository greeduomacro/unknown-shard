...
namespace Server.Gumps
{
	public class ViewInvitedAllegiancesGump : Gump
	{
		...
		public ViewInvitedAllegiancesGump( CityManagementStone stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new ViewInvitedAllegiancesGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}
				...
				if ( m_Stone.AllegiancesInvited.Contains( stone ) )
				{
					...
					{
						m.SendMessage( "CITY MESSAGE: We have declined {0} allegiance offer.", stone.CityName );
					}
				}
				...
				if ( stone.AllegiancesDeclared.Contains( m_Stone ) )
				{
					...
					{
						m.SendMessage( "CITY MESSAGE: {0} has declined our allegiance offer.", m_Stone.CityName );
					}
				}	
			}
				...
				if ( stone.AllegiancesDeclared.Contains( m_Stone ) )
				{
					...
					foreach ( Mobile m in stone.Citizens )
					{
						m.SendMessage( "CITY MESSAGE: We are now allied with {0}.", m_Stone.CityName );
					}
				}	
			}
		}
